...
package net.embest.gnsstestlite
...
import net.embest.gnsstestlite.utils.*
...
class FullscreenActivity : AppCompatActivity() , AdapterView.OnItemSelectedListener {
    ...
    override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
        ...
        if (mTestJob.cep!="") {
            ...
            Log.e(TAG,"CEP:${mTestJob.cep}, ${mTrueLocation!!.latitude},${mTrueLocation!!.longitude}")
        } else {
            Log.e(TAG,"NO CEP")
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                if (mOnNmeaMessageListener == null){
                    mOnNmeaMessageListener = OnNmeaMessageListener { message, _ ->
                        mFile.writeNmeaFile(mRecordFileName + "_" + (mTestJob.count + 1), message)
                        if (!mFirstFixedNmea) {
                    ...
                    mGpsStatusNmeaListener = GpsStatus.NmeaListener { timestamp, message ->
                        ...
                        if (!mFirstFixedNmea) {
                            if (message.indexOf("RMC") > 0) {
                                val messageList = message.split(',')
                                if (messageList[2] == "A") {
                                    mGnssInfo.ttff_nmea = (System.currentTimeMillis() - mStartTimeStamp) / 1000.0f

                                    var nmeaLocation = Location("")

                                    nmeaLocation.latitude = ParseLatitude(messageList[3], messageList[4])
                                    nmeaLocation.longitude = ParseLongitude(messageList[5], messageList[6])

                                    if (mTrueLocation!!.latitude > 360 && mTrueLocation!!.longitude > 360){
                                        mGnssInfo.acc_nmea = 999.0f
                                    }
                                    else{
                                        mGnssInfo.acc_nmea = mTrueLocation!!.distanceTo(nmeaLocation)
                                    }
    ...
    private fun onOneTestDone(firstFix: Boolean){
        Log.e(TAG,"onOneTestDone")
        gpsStop()
        mTestJob.oneTestDone()
        if(firstFix){
            val data = "Index:${mTestJob.count},TTFF:${mGnssInfo.ttff},TTFFNMEA:${mGnssInfo.ttff_nmea},ACC:${mGnssInfo.accuracy},ACC_NMEA:${mGnssInfo.acc_nmea}\r\n"
            mFile.writeResultFile(mRecordFileName, data)
        } else {
            val data = "Index:${mTestJob.count},TTFF:${mTestJob.timeout},TTFFNMEA:${mTestJob.timeout}\r\n"
            mFile.writeResultFile(mRecordFileName, data)
        }
                        ...
                        if (ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                            ...
                            dialog.setMessage(R.string.msg_request_permissions_settings)
                                .setPositiveButton(R.string.msg_dialog_yes) { _, _ ->
                                    startActivity(Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS, Uri.parse("package:net.embest.gnsstestlite")))
                                }
                                .setNegativeButton(R.string.msg_dialog_cancel) { _, _ -> finish() }
                            dialog.show()
                        }
